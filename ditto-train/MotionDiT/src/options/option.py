...
class TrainOptions(PrintableConfig):
    ...
    lip_sync_lambda1: float = 1.0           # weight for L_sync = 1 - cos(A, V_pred)
    lip_sync_lambda2: float = 0.5           # weight for L_stable = |sim_gt - sim_pred|
    lip_sync_every_n_steps: int = 1         # compute lip-sync loss every N steps
    lip_sync_batch_size: int = 4            # number of samples per batch for lip-sync
